...
async def generate_rollout_async(
    ...
    while len(data) < target_data_size:
        ...
        for task in done:
            ...
            logged_sample = _first_sample(group)
            if logged_sample is None:
                metric_gatherer.on_dynamic_filter_drop(reason="empty_custom_generate")
                state.remaining_batch_size -= 1
                continue

            if do_print:
                logger.info(
                    f"First rollout sample: {[str(logged_sample.prompt) + logged_sample.response]}, label: {str(logged_sample.label)[:100]}, reward: {logged_sample.reward}",
                )
                do_print = False
    ...
    all_samples = sorted(
        all_data, key=lambda group: _first_sample(group).index
    )
...
async def eval_rollout_single_dataset(
    ...
    for _i, prompt_sample in enumerate(dataset.samples):
        for j in range(dataset_cfg.n_samples_per_eval_prompt):
            ...
            sample.metadata["_eval_dataset_name"] = dataset_cfg.name
            sample.metadata["_eval_rollout_id"] = rollout_id
...
def generate_rollout(
    ...
    """
    assert args.rollout_global_dataset
    args.alchemy_current_rollout_id = rollout_id
    if evaluation:
