...
def merge(x1: Union[dict, list], x2: Union[dict, list], key: Tuple[Union[str, int], ...] = ()):
    ...
    if isinstance(x1, dict) and isinstance(x2, dict):
        ...
        if _is_optimizer_param_state_key(key):
            # === v3 uniform rule (2026-07-15) — ARCHIVED, offline-verified only ===
            # STATUS: validated offline against real iter_23 common.pt data for all
            # 4 DP-rank layouts (117/122/122/68); NEVER validated in a live resume.
            # Kept for the contingency that cpu-offload must be re-enabled; the
            # adopted mainline fix is dropping --optimizer-cpu-offload entirely
            # (CODEBASE_NO_OFFLOAD=1), which removes this code path's trigger.
            #
            # Background (see notes/CKPT_RESUME_BUGS_0715.md): with the
            # HybridDeviceOptimizer, the dp_reshardable save path leaks per-param
            # ``step`` scalars into the rank-0-only common.pt list, whose length
            # and entry positions are rank-0 quantities. Worse, save-side padding
            # insertion (distrib_optimizer.py:1667) copies the ``step`` key into
            # padding entries as torch.empty() garbage. Positional alignment of
            # the saved list therefore CANNOT be trusted on any rank but the
            # writer. Instead of merging positionally:
            #   - x2 (this rank's local skeleton: per-entry ``padding`` flags,
            #     rebuilt locally via LocalNonpersistentObject) is authoritative
            #     for structure; per-param moments arrive separately via
            #     byte-offset ShardedTensors and are untouched here.
            #   - ``step`` is uniform across params for torch Adam (verified: a
            #     single unique scalar in every ckpt we inspected), so the one
            #     valid scalar found in x1 is stamped onto every entry.
            #   - clone() per entry is REQUIRED: sharing one 0-dim tensor across
            #     entries would make Adam's in-place ``step += 1`` fire once per
            #     param on the same storage, silently exploding the step count.
            # Supersedes the radixark/Megatron-LM PR #66 port (closed unmerged),
            # whose positional truncation branch poisons ranks shorter than the
            # saved list (our 2026-07-15 gate-3 crash: "a Tensor with 32
            # elements cannot be converted to Scalar").
            # NOTE the load pipeline merges INTO this list twice
            # (serialization.py:126 common-vs-padding-skeleton, then :164
            # common-vs-loaded-moments). Length mismatch can only occur on the
            # first merge (adopt the local structure wholesale — the saved
            # positional layout is the writer rank's, not ours); the second
            # merge always sees equal lengths and must merge per-entry to keep
            # locally-rebuilt ``padding`` flags and stamped steps. Stamping the
            # uniform step afterwards is correct in both passes and sanitizes
            # garbage/misaligned steps that positional merging may have kept.
            saved_step = None
            for e in x1:
                if isinstance(e, dict) and "step" in e:
                    s = e["step"]
                    if isinstance(s, (int, float)) or (
                        hasattr(s, "dim") and callable(s.dim) and s.dim() == 0
                    ):
                        saved_step = s
                        break
            if len(x1) != len(x2):
