...
# CODEBASE_TRAIN_ALLOC_CONF injects PYTORCH_CUDA_ALLOC_CONF into the Megatron
# train actors ONLY (via --train-env-vars). Do NOT export the variable globally:
# expandable_segments breaks sglang's CUDA-graph capture / IPC buffers.
# Typical value: expandable_segments:True (fragmentation-OOM mitigation,
# allocator-level only, does not change training math).
if [[ -n "${CODEBASE_TRAIN_ALLOC_CONF:-}" ]]; then
  # No spaces inside the JSON: the arg arrays are expanded unquoted below.
  TRAIN_ENV_VARS_ARG=(--train-env-vars "{\"PYTORCH_CUDA_ALLOC_CONF\":\"${CODEBASE_TRAIN_ALLOC_CONF}\"}")
else
  TRAIN_ENV_VARS_ARG=()
fi
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"CODEBASE_WRITE_WINDOW\": \"${CODEBASE_WRITE_WINDOW:-3}\",
    \"CODEBASE_WRITE_FORMAT_BONUS\": \"${CODEBASE_WRITE_FORMAT_BONUS:-0.1}\",
    \"CODEBASE_TRAIN_ACT_ONLY\": \"${CODEBASE_TRAIN_ACT_ONLY:-}\",
    \"CODEBASE_STEP_GRACE\": \"${CODEBASE_STEP_GRACE:-0}\",
    \"CODEBASE_WRITE_PROMPT_V3\": \"${CODEBASE_WRITE_PROMPT_V3:-}\",
    \"CODEBASE_WRITE_THINKING\": \"${CODEBASE_WRITE_THINKING:-}\",
    \"CODEBASE_ACT_THINKING\": \"${CODEBASE_ACT_THINKING:-}\",
    \"CODEBASE_MULTIBLOCK_FEEDBACK\": \"${CODEBASE_MULTIBLOCK_FEEDBACK:-}\",
    \"CODEBASE_ACT_EXPLORE_BETA\": \"${CODEBASE_ACT_EXPLORE_BETA:-}\",
    \"CODEBASE_JUDGE_API_KEY\": \"${CODEBASE_JUDGE_API_KEY:-}\",
    \"CODEBASE_JUDGE_API_BASE\": \"${CODEBASE_JUDGE_API_BASE:-}\",
    \"CODEBASE_JUDGE_MODEL\": \"${CODEBASE_JUDGE_MODEL:-}\",
    \"CODEBASE_JUDGE_TIMEOUT\": \"${CODEBASE_JUDGE_TIMEOUT:-}\",
    \"CODEBASE_JUDGE_CONCURRENCY\": \"${CODEBASE_JUDGE_CONCURRENCY:-}\",
...
ray job submit --address="http://127.0.0.1:8265" \
  ...
  ${SGLANG_ARGS[@]} \
  ${MISC_ARGS[@]} \
  ${TRAIN_ENV_VARS_ARG[@]+"${TRAIN_ENV_VARS_ARG[@]}"} \
  ${WANDB_ARGS[@]} \
