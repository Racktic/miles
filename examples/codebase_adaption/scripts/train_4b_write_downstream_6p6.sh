#!/usr/bin/env bash
# WRITE=downstream 消融组: R(M_k)=reward[k+1](裸下游, 无 format 无 delta)。
# 与 actonly-6p6 的差异, 仅三点:
#   1. WRITE 参与训练, reward = reward[k+1](裸下游, GRPO 组内归一化承担难度)
...
export CODEBASE_WRITE_REWARD_MODE=downstream
export CODEBASE_NO_OFFLOAD=1
export CODEBASE_SGLANG_RECAPTURE_PATCH=1        # 指纹仪表+recapture, 保留
export TORCH_NCCL_HEARTBEAT_TIMEOUT_SEC=3600

export CODEBASE_RUN_ID="swecl-4b-write-downstream-6p6"
