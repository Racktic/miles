#!/usr/bin/env bash
# lr=0 金丝雀 #2: --disable-cuda-graph 判别实验(2026-07-14)。
# 金丝雀 #1(recapture 补丁)已证明: 重捕获正确触发但 rollout_1 照崩(病理逐项与无补丁时相同),
# 即 #27140 机制不是我们的病因。本实验彻底禁用 CUDA graph(decode 全走 eager):
#   rollout_1 恢复健康 -> graph 仍有罪, 只是 recapture 覆盖不到的部分(如输入 buffer 绑定);
#   rollout_1 照崩     -> graph 全线无罪, 转向 mamba cache 机制(#24954/#26941, --disable-radix-cache)。
# 与金丝雀 #1 的唯一差异 = CODEBASE_SGLANG_EXTRA_ARGS(recapture bind 保留, 无 graph 时空转)。
set -euo pipefail
SD="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")/.." && pwd)"

export CODEBASE_SGLANG_RECAPTURE_PATCH=1   # 保留(无graph时自动空转), 保证与金丝雀#1单变量差异
export CODEBASE_SGLANG_EXTRA_ARGS="--sglang-disable-cuda-graph"   # 本实验唯一自变量

export CODEBASE_RUN_ID="swecl-4b-nograph-lr0"
