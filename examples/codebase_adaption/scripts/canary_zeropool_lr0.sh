#!/usr/bin/env bash
# lr=0 金丝雀 #4(备用): 醒后霰弹清零(post-resume shotgun zero)(2026-07-14)。
# 依据: region 内张量醒后内容=垃圾; 常规安全靠"先写后读", 例外是 MambaPool 的 padding
# slot 0(free_slots=arange(1,size+1), 永不 re-alloc 清零, kernel 对 padded 位置写/读它)。
# 本实验在 mixin 的 post-resume 钩子里追加: req_to_token.zero_() + mamba conv/temporal 全量
# zero_()(含 slot 0)。与 #3 的唯一差异 = 这个 shotgun zero(需 mixin 对应 commit)。
set -euo pipefail
...
export CODEBASE_RUN_ID="swecl-4b-zeropool-lr0"
