#!/usr/bin/env bash
# v3nocurr gated-downstream run (2026-07-30, user-specified):
...
#     (~50% easy / 34% mid / 16% hard instances; expected success ~26%,
#     positive anchors never starve). No macro curriculum, no tier cliffs.
#   - reward: official step discount (no STEP_GRACE), WRITE trained with
#     gated_downstream + format bonus — the smith-v2 / babel-v3curr recipe.
#   - CODEBASE_DROP_ZERO_STD_GROUPS=1 (with min_keep batch guard in the
#     filter). NOTE: the v3nocurr pair (this run vs actonly) differs ONLY in
#     CODEBASE_TRAIN_ACT_ONLY, so the filter is ON in both.
# Counterpart: train_4b_v3nocurr_actonly_v3think.sh (WRITE untrained).
# Usage: nohup bash scripts/train_4b_v3nocurr_gated.sh > logs/smith-4b-v3nocurr-gated.console.log 2>&1 &
set -euo pipefail
...
export CODEBASE_RUN_ID="smith-4b-v3nocurr-gated"
...
cd "$SD"
echo "=== V3NOCURR GATED RUN=${CODEBASE_RUN_ID} on $(hostname) (ckpt/traj: ${NODE_RUN_DIR}) ==="
