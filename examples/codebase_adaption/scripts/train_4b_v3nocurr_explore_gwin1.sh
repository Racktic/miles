#!/usr/bin/env bash
# v3nocurr explore-gwin1 run (2026-08-13, user-specified): K=1 ablation of the
# explore-gwin arm — identical except CODEBASE_WRITE_WINDOW=1.
# NOTE the accepted caveat: K=1 has the strongest mean-reversion bias in the
# delta family (corr -0.736, WRITE_COLLAPSE_ANALYSIS_0714); this run measures it.
...
# Usage: nohup bash scripts/train_4b_v3nocurr_explore_gwin1.sh > /tmp/qixinx/smith-4b-v3nocurr-explore-gwin1.console.log 2>&1 &
set -euo pipefail
...
export CODEBASE_WRITE_WINDOW=1
...
export CODEBASE_RUN_ID="smith-4b-v3nocurr-explore-gwin1"
# Date-stamped, decoupled from RUN_ID: an existing wandb id would resume into the old run.
export CODEBASE_WANDB_RUN_ID="smith-4b-v3nocurr-explore-gwin1-0813"
...
cd "$SD"
echo "=== V3NOCURR EXPLORE-GWIN1 RUN=${CODEBASE_RUN_ID} on $(hostname) (ckpt/traj: ${NODE_RUN_DIR}) ==="
