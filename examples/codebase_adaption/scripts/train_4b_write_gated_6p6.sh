#!/usr/bin/env bash
# WRITE=gated 消融组(严格 format, 2026-07-17): R(M_k)=format_ok×(reward[k+1]+0.1)。
# 与 actonly-6p6 的差异, 仅三点:
#   1. WRITE 参与训练, reward = format_ok × (reward[k+1] + 0.1); format=严格判据(各恰1标题+每节≥2bullet+无prompt回显+不与上版重复)
#      (CODEBASE_WRITE_REWARD_MODE=gated_downstream; format=两节标题存在性;
#       无 delta —— 防均值回归 bias, 题目难度由 GRPO 组内归一化承担);
...
export CODEBASE_RUN_ID="swecl-4b-write-gated-6p6"
export CODEBASE_WANDB_RUN_ID="${CODEBASE_RUN_ID}-v3"   # v2(严格format首跑)死于节点过期已占用原id, 新跑用干净id
