#!/usr/bin/env bash
# gwin K=3 + exploration reward + ACT thinking, "cispo" RL-config arm (2026-08-18,
# user-specified). Byte-for-byte the explore-gwin-think setting except three
# RL-config changes:
#   1. Train-infer mismatch correction (CISPO-style): sampler log-probs are
#      attached to ACT/WRITE samples and miles' TIS multiplies the policy loss by
#      clamp(exp(logp_train - logp_sampler), 0, 2), detached (miles
#      corrections.py::vanilla_tis_function). PPO clip stays a no-op (old
#      log-probs still recomputed by the trainer; --use-rollout-logprobs NOT set).
#      Also emits tis / tis_abs / tis_clipfrac to wandb.
#   2. KL to reference: estimator k1 -> k3, coefficient 0.01 -> 0.001
#      (k1 as a loss has zero-mean gradient; k3 actually regularises).
#   3. fp32 LM head on the sampler (--sglang-enable-fp32-lm-head); trainer
#      logits were already fp32.
# Everything else (GRPO, eps 0.2/0.28, entropy 0, lr 1e-6, batch 2x8, SEQ_LEN =
# MAX_TOK = 20480, explore beta 0.3, gated_windowed K=3, ACT/WRITE thinking,
# MULTIBLOCK_FEEDBACK, DROP_ZERO_STD_GROUPS, 191/8/4) is inherited unchanged.
# Usage: nohup bash scripts/train_4b_v3nocurr_explore_gwin_think_cispo.sh > /tmp/qixinx/smith-4b-v3nocurr-explore-gwin-think-cispo.console.log 2>&1 &
set -euo pipefail
...
# --- the three RL-config deltas vs explore-gwin-think ---
# (1) CISPO-style truncated importance sampling on the sampler/trainer log-prob
#     ratio; codebase_rollout attaches sampler log-probs when --use-tis is on.
# (2) KL estimator k3 (overrides the k1 in run_codebase_adaption_qwen3.5_4B.sh;
#     argparse keeps the last occurrence) with coefficient 0.001.
export CODEBASE_KL=0.001
export CODEBASE_TRAIN_EXTRA_ARGS="--kl-loss-type k3 --use-tis --tis-clip 2 --tis-clip-low 0"
# (3) fp32 LM head in the sglang sampler.
export CODEBASE_SGLANG_EXTRA_ARGS="--sglang-enable-fp32-lm-head"

export CODEBASE_RUN_ID="smith-4b-v3nocurr-explore-gwin-think-cispo"
# Date-stamped, decoupled from RUN_ID: an existing wandb id would resume into the old run.
export CODEBASE_WANDB_RUN_ID="smith-4b-v3nocurr-explore-gwin-think-cispo-0818"
...
cd "$SD"
echo "=== V3NOCURR EXPLORE-GWIN-THINK-CISPO RUN=${CODEBASE_RUN_ID} on $(hostname) (ckpt/traj: ${NODE_RUN_DIR}) ==="
