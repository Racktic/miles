...
CANDIDATES_PER_PROBLEM="${FRONTIERCS_CANDIDATES_PER_PROBLEM:-4}"
GROUP_SIZE="${FRONTIERCS_GROUP_SIZE:-3}"
GROUPS_PER_UPDATE="${FRONTIERCS_GROUPS_PER_UPDATE:-2}"
TRAIN_WRITE="${FRONTIERCS_TRAIN_WRITE:-1}"
WRITE_ADVANTAGE_MODE="${FRONTIERCS_WRITE_ADVANTAGE_MODE:-grpo}"
WRITE_CANDIDATES="${FRONTIERCS_WRITE_CANDIDATES:-4}"
...
ACT_ADVANTAGE_MODE="${FRONTIERCS_ACT_ADVANTAGE_MODE:-group_relative}"
...
if ! [[ "${WRITE_CANDIDATES}" =~ ^[1-9][0-9]*$ ]]; then
  echo "FRONTIERCS_WRITE_CANDIDATES must be a positive integer" >&2
  exit 2
fi
case "${TRAIN_WRITE}" in
  0|1) ;;
...
case "${WRITE_ADVANTAGE_MODE}" in
  direct|positive_only|center_by_round)
    if [ "${WRITE_CANDIDATES}" -ne 1 ]; then
      echo "FRONTIERCS_WRITE_CANDIDATES must be 1 unless FRONTIERCS_WRITE_ADVANTAGE_MODE=grpo" >&2
      exit 2
    fi
    ;;
  grpo)
    if [ "${TRAIN_WRITE}" -ne 1 ] || [ "${WRITE_CANDIDATES}" -lt 2 ]; then
      echo "WRITE GRPO requires FRONTIERCS_TRAIN_WRITE=1 and FRONTIERCS_WRITE_CANDIDATES>=2" >&2
      exit 2
    fi
    ;;
  *)
    echo "FRONTIERCS_WRITE_ADVANTAGE_MODE must be direct, positive_only, center_by_round, or grpo" >&2
    exit 2
    ;;
esac
case "${ACT_ADVANTAGE_MODE}" in
  raw|task_baseline|temporal_problem_relative)
    ;;
  group_relative)
    if [ "${CANDIDATES_PER_PROBLEM}" -lt 2 ]; then
      echo "${ACT_ADVANTAGE_MODE} requires FRONTIERCS_CANDIDATES_PER_PROBLEM>=2" >&2
      exit 2
    fi
    ;;
  *)
    echo "FRONTIERCS_ACT_ADVANTAGE_MODE must be raw, task_baseline, temporal_problem_relative, or group_relative" >&2
    exit 2
    ;;
esac
...
echo "groups=${GROUP_COUNT} G=${GROUP_SIZE} groups_per_update=${GROUPS_PER_UPDATE} rounds=${MEMORY_ROUNDS} K_act=${CANDIDATES_PER_PROBLEM} act_advantage=${ACT_ADVANTAGE_MODE} K_write=${WRITE_CANDIDATES} train_write=${TRAIN_WRITE} write_advantage=${WRITE_ADVANTAGE_MODE}"
...
SAMPLES_PER_EPISODE=$((GROUP_SIZE * MEMORY_ROUNDS * CANDIDATES_PER_PROBLEM + TRAIN_WRITE * WRITE_CANDIDATES * (MEMORY_ROUNDS - 1)))
...
RUNTIME_ENV_JSON="{
  \"env_vars\": {
    ...
    \"FRONTIERCS_ACT_ADVANTAGE_MODE\": \"${ACT_ADVANTAGE_MODE}\",
