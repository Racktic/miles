#!/bin/bash
# Symbolic Alchemy two-phase (ACT->REWRITE) TEXT rollout smoke on Qwen3.5-4B (TP=#GPUs, colocate).
...
# Qwen3.5-4B megatron spec -> MODEL_ARGS
...
# HF checkpoint (tokenizer + the megatron->sglang weight bridge). The stock public Qwen3.5-4B is a VL+MTP
...
CKPT=/data/user_data/qixinx/Qwen3.5-4B
...
TORCH_DIST=/data/user_data/qixinx/Qwen3.5-4B_torch_dist
...
if [ -n "${WANDB_API_KEY}" ] && [ -n "${ALCHEMY_USE_WANDB:-}" ]; then
  WANDB_ARGS=(--use-wandb --wandb-project "${WANDB_PROJECT:-miles-alchemy}" --wandb-group "${WANDB_GROUP:-qwen3.5-4B}" --wandb-key "${WANDB_API_KEY}")
  echo "wandb: ENABLED"
else
...
PERF_ARGS=(
   ...
   # too small and silently truncates late-trial turns. Qwen3.5-4B is natively 262144-ctx and uses RoPE,
